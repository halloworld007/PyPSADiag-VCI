...
class DiagnosticCommunication(QThread):
    ...
    def writeECUCommand(self, cmd: str):
        ...
        # Check if using VCI adapter
        if hasattr(self.serialPort, 'use_vci') and self.serialPort.use_vci:
            ...
            if cmd.startswith(">") and ":" in cmd:
                ...
                self.writeToOutputView("< VCI: ECU already configured, skipping Arduino command")
                return "OK"
            elif cmd.startswith("L"):
                # LIN command - skip for VCI (not supported yet)
                self.writeToOutputView("< VCI: LIN not supported, skipping")
                return "OK"
            elif cmd == "R":
                # Reset command - skip for VCI
                self.writeToOutputView("< VCI: Reset not needed, skipping")
                return "OK"
            else:
                # Regular hex command - send to VCI
                receiveData = self.serialPort.sendReceive(cmd)
        else:
            # Arduino mode: Send command as-is
            receiveData = self.serialPort.sendReceive(cmd)
    ...
    def startSendingKeepAlive(self):
        # Check if using VCI adapter
        if hasattr(self.serialPort, 'use_vci') and self.serialPort.use_vci:
            self.writeToOutputView("< VCI: Keep-alive handled automatically")
            return True
    ...
    def stopSendingKeepAlive(self):
        # Check if using VCI adapter
        if hasattr(self.serialPort, 'use_vci') and self.serialPort.use_vci:
            self.writeToOutputView("< VCI: Keep-alive stop not needed")
            return True
    ...
    def startDiagnosticMode(self):
        # Check if using VCI adapter
        if hasattr(self.serialPort, 'use_vci') and self.serialPort.use_vci:
            # VCI: Send actual diagnostic session start command
            receiveData = self.writeECUCommand("1003")  # UDS Diagnostic Session Control
            if len(receiveData) >= 4 and receiveData[:4] == "5003":
                self.writeToOutputView("< VCI: Diagnostic session started")
                return True
            else:
                self.writeToOutputView("< VCI: Using existing diagnostic session")
                return True  # Assume already in diagnostic mode
